...
- [API接口加密验证](#a1)
    ...
    - [标准规范](#a9)
        ...
        - [限流 - REST API](#b4)
- [业务API参考](#b5)
    - [open-api](#b6) ([调用 Demo](spot-api/demo.txt))
  -   [资产余额](#1)
  -   [获取全部委托](#2)
  -   [获取全部成交记录](#3)
  -   [取消委托单](#4)
...
主要常用接口：
- 获取余额接口 open/api/user/account
- 获取成交记录接口 open/api/all_trade
- ticker行情接口 open/api/market_dept
- 订单详情 open/api/order_info
- 批量下单撤单 open/api/mass_replaceV2
- 按币对全部撤单 open/api/cancel_order_all
- 当前所有委托订单 open/api/v2/all_order
- ticker接口 open/api/get_ticker
- 币对信息 open/api/common/symbols
- 自成交接口 open/api/self_trade
...
生成待签名的字符串
-   [open-api Demo](spot-api/demo.java)
...
REST访问的根URL：``` https://test-ibit-open.aiby2.com ```
...
* 单币对盘口铺单最大数量为5000单，超过该数量将拒绝做市下单。
...
* mass_place的铺单总数最多5000条，超过将拒绝下单
...
---
###  <span id="19">自成交订单</span>


自成交接口使用通用的接口签名规则，post请求。
接口作用：调用自成交接口，可以不下单的情况下产生成交记录，避免摆盘被薅羊毛，成交信息会出现在交易页k线上。接口会随机制定主动单方向。
